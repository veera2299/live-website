...
const Modify_events = () => {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        const response = await fetch(`${API_URL}/all-events`);
        const data = await response.json();
  ...
  const handleDelete = async (id) => {
    ...
    if (window.confirm("Are you sure you want to delete this event?")) {
      try {
        // REVERTED: Using your original route structure
 ...
 // 3. Handle Edit
  const handleEdit = (id) => {
    navigate(`/admin/modify_events/${id}`);
  };
          ...
          {filteredEvents.map((event) => (
            <div key={event._id} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow duration-300 flex flex-col">
              ...
              <div className="h-52 bg-gray-100 relative group">
                {event.eventImages && event.eventImages.length > 0 ? (
                   <Slider {...sliderSettings}>
                     {event.eventImages.map((publicId, index) => (
                       <div key={index} className="h-52 outline-none">
                         ...
                         <img 
                           ...
                           alt={`Slide ${index}`} 
                           className="w-full h-full object-cover"
                           onError={(e) => {e.target.src = "https://dummyimage.com/400x200/cccccc/000000&text=No+Image"}}
                         />
                       </div>
                     ))}
                   </Slider>
